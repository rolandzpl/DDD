...
namespace DDD.Domain
{
	class FileEventStoreTests
	{
		...
		class SavingOrReading
		{
			...
			{
				StoreEventDataInFilesystem(DateTime.UtcNow.AddTicks(100), aggregateId, 0);
				var events = GetEvents();
			...
			public void GetAllEvents_SomeEventsExistInTheSystem_ReturnsEventsOrderredByTimestamp()
			{
				...
				var eventId1 = StoreEventDataInFilesystem(now.AddTicks(100), aggregateId1, 0);
				var eventId2 = StoreEventDataInFilesystem(now.AddTicks(101), aggregateId2, 0);
				var eventId3 = StoreEventDataInFilesystem(now.AddTicks(102), aggregateId1, 1);
			...
			{
				var now = DateTime.UtcNow;
				var eventId = StoreEventDataInFilesystem(now.AddTicks(100), aggregateId, 0);
			...
			{
				var now = DateTime.UtcNow;
				StoreEventDataInFilesystem(now.AddTicks(100), aggregateId, 0);
				var e = eventStore.GetEvent(Guid.NewGuid());
				Assert.That(e, Is.Null);
			}

			private Guid StoreEventDataInFilesystem(DateTime now, Guid aggregateId, int version)
			{
